...
class Instrument:
	def __init__(self, f):
		self.name = f.read(22).decode("latin_1").rstrip('\0')
...
class Module:
	def __init__(self, f):
		self.name = f.read(20).decode("latin_1").rstrip('\0')
...
if len(raw_instruments) > 0:
	...
	insts_data += bytes().join(inst_data[:len(raw_instruments)])
